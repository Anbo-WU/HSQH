...
def windows_desktop_folder() -> Path:
    """读取 Windows 配置的桌面目录，兼容 OneDrive 和目录重定向。"""
    if os.name != "nt":
    ...
    if result != 0 or not buffer.value:
        raise RuntimeError("Windows 无法解析当前用户的桌面目录。")
...
def latest_completed_batch(teacher: str) -> tuple[date, int] | None:
    ...
    if not candidates:
        return None
    _, task_date, batch = max(candidates)
    return task_date, batch
...
def build_task_paths(
    ...
    scan_source_pdf = scan_source.expanduser() if scan_source is not None else Path(
        f"{effective_scan_date:%m%d}.pdf"
    )
    if not scan_source_pdf.is_absolute():
        scan_source_pdf = scanned_folder / scan_source_pdf
    scan_source_pdf = scan_source_pdf.resolve()
    split_folder_name = (
    ...
    return TaskPaths(
        ...
        scan_source_pdf=scan_source_pdf,
        scan_source_is_override=scan_source is not None,
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        type=Path,
        metavar="PDF",
        help="仅用于 B 测试/排障；指定后覆盖当天的 MMDD.pdf",
    )
