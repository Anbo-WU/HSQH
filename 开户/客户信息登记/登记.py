...
def financial_value(lines: list[Line], label: str, header: str) -> tuple[float, list[Line]]:
    ...
    for group in rows(lines):
        for line in group:
            text = compact(line.text)
            is_label = text == label if label != '净利润' else bool(re.match(r'^(?:[一二三四五六七八九十]+、)?净利润(?:[（(]|$)', text))
            if not is_label:
                continue
            headings = [x for x in lines if compact(x.text) == header and x.cy < line.cy and x.x0 > line.x0]
            if not headings:
                continue
            heading = min(headings, key=lambda x: x.x0)
            peer_headers = [x for x in lines if abs(x.cy-heading.cy) < 0.01 and
                            (compact(x.text) in {'行次', '期末余额', '年初余额', '本期金额', '本年累计金额'})]
            center = (heading.x0 + heading.x1)/2
            left = max(((x.x0+x.x1)/2 for x in peer_headers if (x.x0+x.x1)/2 < center), default=line.x1)
            right = min(((x.x0+x.x1)/2 for x in peer_headers if (x.x0+x.x1)/2 > center), default=2-center)
    ...
    unit = re.search(r'单位[:：]?(?:人民币)?(万元|千元|元)', unit_text)
    if not unit:
        raise ValueError('财报计量单位未识别，不能确认金额为元')
    value *= {'元': 1, '千元': 1000, '万元': 10000}[unit[1]]
...
def read_kyc(folder: Path) -> tuple[dict[str, Field], dict[str, Field]]:
    ...
    try:
        for target, col, sheet, cell in mapping:
            ...
            try:
                item = get_sheet(book, sheet)[cell]
                value = item.value
...
def collect_customer(folder: Path, reader: PDFReader, scope_source: str) -> tuple[dict, dict]:
    ...
    def license_fields():
        ...
        if scope_source == 'license':
            try:
                anchor = next(x for x in lines if compact(x.text) == '经营范围')
                chosen = [x for x in lines if x.x0 >= anchor.x1-0.005 and x.x0 < 0.55 and anchor.cy-0.015 <= x.cy < anchor.cy+0.20]
                chosen.sort(key=lambda x: (x.cy, x.x0))
                text = ''.join(x.text for x in chosen)
                if not text or '经营活动' not in text:
                    raise ValueError('营业执照经营范围未识别完整')
                note = '来源暂按营业执照；请核对扫描识别文字'
                # OCR 经常将分号识别成冒号或漏括号。仅在去标点后文字完全一致时校对标点。
                try:
                    kyc_path = unique_file(folder/'0.OA', lambda p: 'KYC' in p.name.upper() and p.suffix.lower() == '.xlsx')
                    kyc_book = load_workbook(kyc_path, data_only=True)
                    try:
                        reference = str(get_sheet(kyc_book, '基本信息表')['C14'].value or '')
                    finally:
                        kyc_book.close()
                    without_punctuation = lambda s: re.sub(r'[^\w]', '', compact(s))
                    if reference and without_punctuation(reference) == without_punctuation(text):
                        text = reference
                        note = '营业执照 OCR 与 KYC 基本信息表 C14 去标点后完全一致；使用 C14 校对标点'
                except (ValueError, OSError):
                    pass
                result['N'] = extracted(text, f'{path.name} / 经营范围', chosen, note)
            except Exception as exc:
                result['N'] = Field(source=path.name, status='待核对', note=str(exc))
        return result

    def finance(col, page, label, heading):
        path = document(folder, 16)
        value, evidence = financial_value(reader.page(path, page), label, heading)
        return {col: extracted(value, f'{path.name} / 第{page+1}页 / {label} / {heading}', evidence)}
    ...
    def credit():
        ...
        scores, evidence = [], []
        for group in rows(second):
            for i, line in enumerate(group):
                if '综合评分' not in compact(line.text) or re.search(r'综合评分\d', compact(line.text)):
                    continue
                tail = compact(line.text).split('综合评分',1)[1].lstrip(':：')
                candidate = [(tail, line)] if tail else [(compact(x.text), x) for x in group[i+1:] if x.x0 < .63]
                found = [(float(text), item) for text, item in candidate if re.fullmatch(r'-?\d{1,3}(?:\.\d+)?', text)]
                if len(found) == 1 and -100 <= found[0][0] <= 105:
                    scores.append(found[0][0])
                    evidence.extend([line, found[0][1]])
        if scores and len(set(scores)) == 1:
            result['BC'] = extracted(scores[0], f'{path.name} / 第2页综合评分', evidence, '多处综合评分一致' if len(scores)>1 else '仅识别到一处综合评分')
        else:
            result['BC'] = Field(source=path.name, status='待核对', note=f'综合评分未识别或多处不一致：{scores}')
        return result

    task(['K'], '1.交易者基本信息表', nature)
    task(['R'], '13.受益人信息采集表', beneficiaries)
    task(['AC']+(['N'] if scope_source == 'license' else []), '14.营业执照', license_fields)
    if scope_source == 'blank':
        main['N'] = Field(source='未指定来源', status='待核对', note='经营范围暂留空')
    for col, page, label, heading in [('AD', 0, '资产合计', '期末余额'), ('AE', 0, '负债合计', '期末余额'), ('AG', 1, '净利润', '本年累计金额')]:
        task([col], '16.财报表', lambda c=col, p=page, l=label, h=heading: finance(c, p, l, h))
...
def main() -> int:
    ...
    parser.add_argument('--scope-source', choices=['license', 'blank'], default='license', help='经营范围来源，默认营业执照')
    ...
    for folder in folders:
        ...
        main_fields, auth = collect_customer(folder, reader, args.scope_source)
